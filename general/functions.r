...
runModel <- function(sampleID, outType="dTabs", uncRCP=0, segScen="Base",
  ...
  if(regSets!="maakunta"){
    ...
    if(toRaster){
      ...
      print(paste("TotArea=",sum(data.all$area),", no harvest fract % =",
                  round(sum(data.all$area[data.all$landclass > 1  | data.all$cons==1])/sum(data.all$area)*100,2)))
      print(paste("SampleArea=",sum(data.sample$area),", no harvest fract % =",
                  round(sum(data.sample$area[data.sample$landclass > 1  | data.sample$cons==1])/sum(data.sample$area)*100,2)))
    } else {
  ...
  #print(c(harvScen,harvInten,initilizeSoil,rcps,nYears))
  if(harvScen=="Base" & harvInten =="Base" & initilizeSoil & rcps=="CurrClim" & nYears<40){
